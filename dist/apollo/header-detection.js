...
exports.GRAPHQL_TRAFFIC_HEADER_NAMES = void 0;
exports.isAllowedGraphqlTrafficHeader = isAllowedGraphqlTrafficHeader;
exports.filterGraphqlTrafficHeaders = filterGraphqlTrafficHeaders;
exports.parseGraphqlRequestBody = parseGraphqlRequestBody;
exports.normalizeVariablesJson = normalizeVariablesJson;
exports.isTrivialProbeQuery = isTrivialProbeQuery;
exports.hasRealGraphqlOperation = hasRealGraphqlOperation;
...
exports.mergeTrafficHeadersOnly = mergeTrafficHeadersOnly;
exports.mergeTrafficCapture = mergeTrafficCapture;
exports.isTrafficHeaderSource = isTrafficHeaderSource;
exports.isOperationCaptureSource = isOperationCaptureSource;
exports.buildReadCachedGraphqlRequestScript = buildReadCachedGraphqlRequestScript;
exports.buildInstallPersistentTrafficHookScript = buildInstallPersistentTrafficHookScript;
exports.hasRequiredGraphqlAuthHeaders = hasRequiredGraphqlAuthHeaders;
...
exports.GRAPHQL_TRAFFIC_HEADER_NAMES = new Set([
    "authorization",
    "x-company-id",
...
function isAllowedGraphqlTrafficHeader(name) {
    const lower = name.toLowerCase();
    if (exports.GRAPHQL_TRAFFIC_HEADER_NAMES.has(lower))
        return true;
    if (lower.startsWith("x-datadog-"))
        return false;
    if (lower.startsWith("sec-"))
        return false;
    return false;
}
function filterGraphqlTrafficHeaders(headers) {
    const filtered = {};
    for (const [key, value] of Object.entries(headers)) {
        if (!value || SKIP_REQUEST_HEADERS.has(key.toLowerCase()))
            continue;
        if (!isAllowedGraphqlTrafficHeader(key))
            continue;
        filtered[key] = value;
    }
    return filtered;
...
function isTrivialProbeQuery(query) {
    if (!query?.trim())
        return true;
    const normalized = query.replace(/\s+/g, " ").trim();
    if (normalized.includes("ApolloSandboxProbe"))
        return true;
    if (/__schema\b|IntrospectionQuery|query\s+Introspection/i.test(normalized)) {
        return true;
    }
    return /^(\{\s*__typename\s*\}|query\s+\w*\s*\{\s*__typename\s*\})$/i.test(normalized);
}
function hasRealGraphqlOperation(capture) {
    return Boolean(capture?.operation?.trim() && !isTrivialProbeQuery(capture.operation));
}
function pickRicherOperation(current, part) {
...
function formatPartVariablesJson(part) {
...
function buildAutoDetectHeadersScript(graphqlUrl, urlMatch, listenMs) {
    ...
    const allowList = JSON.stringify([...exports.GRAPHQL_TRAFFIC_HEADER_NAMES]);
    return `(async () => {
...
function buildPersistHeadersScript(capture, meta) {
...
function mergeTrafficHeadersOnly(...parts) {
    const trafficParts = parts.filter((part) => part &&
...
function mergeTrafficCapture(...parts) {
    const merged = mergeTrafficHeadersOnly(...parts);
    for (const part of parts) {
        if (!part)
            continue;
        const fromCapture = part.sources?.some(isTrafficHeaderSource) ||
...
function isTrafficHeaderSource(source) {
    return source === "traffic" || source === "apollo-link";
}
function isOperationCaptureSource(source) {
...
function buildReadCachedGraphqlRequestScript() {
    return `(() => {
  const isTrivialProbe = (query) => {
    if (!query || typeof query !== 'string') return true;
    const n = query.replace(/\\s+/g, ' ').trim();
...
function buildInstallPersistentTrafficHookScript() {
    const allowList = JSON.stringify([...exports.GRAPHQL_TRAFFIC_HEADER_NAMES]);
...
function hasRequiredGraphqlAuthHeaders(headers) {
    const lower = Object.fromEntries(Object.entries(headers).map(([k, v]) => [k.toLowerCase(), v]));
    return Boolean(lower.authorization &&
        lower["x-company-id"] &&
